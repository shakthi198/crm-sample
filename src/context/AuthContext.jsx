...
export const AuthProvider = ({ children }) => {
  ...
  const hasPermission = (permission) => {
    ...
    const modulePerm = user.permissions?.find(
      (p) => p.module.toLowerCase() === permission.toLowerCase(),
    );
    if (modulePerm) {
      return (
        parseInt(modulePerm.view) === 1 ||
        parseInt(modulePerm.full_access) === 1
      );
    }
  ...
  return (
    <AuthContext.Provider
      value={{ user, login, logout, switchRole, hasPermission, updateProfile }}
    >
      {children}
    </AuthContext.Provider>
  );
