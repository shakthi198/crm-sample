  ...
  const normalizeToken = (rawToken) => {
    ...
    if (cleanToken.startsWith("{") && cleanToken.endsWith("}")) {
      try {
        ...
        cleanToken = parsed?.token || parsed?.access_token || parsed?.jwt || "";
      } catch {
        return "";
      }
    }
    ...
    cleanToken = String(cleanToken)
      ...
      .replace(/^['"]+|['"]+$/g, "")
      .trim();
    while (/^bearer\s+/i.test(cleanToken)) {
      ...
      {loading && !data ? (
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          <Grid container spacing={3}>
            {(loading ? Array.from({ length: 4 }) : kpiCards).map(
              (card, index) => (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          <Legend
                            ...
                            iconType="circle"
                            wrapperStyle={
                              isMobile ? { paddingTop: "20px" } : undefined
                            }
                          />
                        </PieChart>
                      </ResponsiveContainer>
          ...
          <Box ref={followUpsRef}>
            ...
            >
              ...
              >
                ...
                <Chip
                  label={`${data?.today_followups_table?.length || 0} Pending`}
                  size="small"
                  color="primary"
